...
function LikedPokemons() {
  ...
  return (
    ...
    {pokemons.filter(pok => pok.isLiked).length ? (
      pokemons.filter(pok => pok.isLiked).map((pokemon, index) => (
        <PokemonCard
          key={index}
          id={pokemon.id}
          img={pokemon.img}
    ...
    ) : <h1 className='text-neutral-500 font-bold text-4xl w-full h-full flex items-center justify-center'>Not Yet Liked Todo</h1>}
  </section>
  )
}
